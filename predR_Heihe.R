...
wdPath <- "/Users/Documents/R/statFor";
dataPath <- "/Users/Documents/R/statFor/data";
#setwd(wdPath)
...
# DATA ACQUISITION ----
# DISCHARGE Q
qZ <- read.csv(file="~/Documents/R/statFor/data/Zhamashike_runoff.csv", header=TRUE, sep=",")
qQ <- read.csv(file="~/Documents/R/statFor/data/Qilian_runoff.csv", header=TRUE, sep=",")
qY <- read.csv(file="~/Documents/R/statFor/data/Yingluoxia_runoff.csv", header=TRUE, sep=",")
qZ <- qZ[,-1]
...
qY <- na.omit(qY)
# SNOW
sc.Q <- read.csv(file="/Users/beamarti/Documents/R/statFor/data/JulesQilian.csv", header=TRUE, sep=",")
sc.Z <- read.csv(file="/Users/beamarti/Documents/R/statFor/data/JulesZhamashike.csv", header=TRUE, sep=",")
plot(sc.Q); plot(sc.Z) # visualization
# Correlation between catchments?
plot(log(sc.Q[,4]),log(sc.Z[,4])); grid(); abline(0,1,col="red") # this is the most important data
# WMO TEMPERATURE DATA
temp.Zhanye <- read.csv(file="/Users/beamarti/Documents/R/statFor/data/526520.txt", header=TRUE, sep=",")
...
# TOPOGRAPHY DATA
srtmTile <- getData('SRTM', lon=100, lat=38, path="data")
srtmTileW <- getData('SRTM', lon=98, lat=38, path="data")
...
# SHAPEFILES
shape.Z <- readOGR(dsn="/Users/beamarti/Documents/R/statFor/data",layer="catchment_zhamashike_2")
shape.Q <- readOGR(dsn="/Users/beamarti/Documents/R/statFor/data",layer="catchment_qilian")
shape.Y <- readOGR(dsn="/Users/beamarti/Documents/R/statFor/data",layer="catchment_yingluoxia_final")
shape.Z <- spTransform(shape.Z, CRS("+proj=longlat +datum=WGS84"))
shape.Q <- spTransform(shape.Q, CRS("+proj=longlat +datum=WGS84"))
shape.Y <- spTransform(shape.Y, CRS("+proj=longlat +datum=WGS84"))

# ALREADY DONE - XOR Shapes
# shape.Y <- shape.Y-shape.Q - shape.Z# just do Yingluosha excluding the two other catchment parts
# writeOGR(shape.Y, wdPath, "catchment_yingluoxia_final", driver="ESRI Shapefile")

map.Q <- intersect(srtmmosaic,shape.Q)
map.Z <- intersect(srtmmosaic,shape.Z)
map.Y <- intersect(srtmmosaic,shape.Y)
lines(shape.Q,col="red")
lines(shape.Z,col="red")
lines(shape.Y,col="red")
#plot(map.Z)

# Station lat/lon
statLoc.Y.Lat <- 38.8; statLoc.Y.Lon <- 100.1833;
statLoc.Q.Lat <- 38.2; statLoc.Q.Lon <- 100.2333;
statLoc.Z.Lat <- 38.2333; statLoc.Z.Lon <- 99.9833;

statLoc <- data.frame(Latitude=c(statLoc.Y.Lat,statLoc.Q.Lat,statLoc.Z.Lat),Longitude=c(statLoc.Y.Lon,statLoc.Q.Lon,statLoc.Z.Lon))

# MAPPING DATA
shape.Q.fortified <- fortify(shape.Q)
shape.Z.fortified <- fortify(shape.Z)
shape.Y.fortified <- fortify(shape.Y)
qmap("Zhangye",zoom=7) +
  geom_polygon(aes(x = long, y = lat, group = group), data = shape.Q.fortified,
               colour = 'white', fill = 'red', alpha = .4, size = .3) +
  geom_polygon(aes(x = long, y = lat, group = group), data = shape.Z.fortified,
               colour = 'white', fill = 'red', alpha = .4, size = .3) +
  geom_polygon(aes(x = long, y = lat, group = group), data = shape.Y.fortified,
               colour = 'white', fill = 'red', alpha = .4, size = .3) +
  geom_point(data = statLoc, aes(x = Longitude, y = Latitude), color="blue", size=2, alpha=0.5)
